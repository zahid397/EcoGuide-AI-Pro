    ...
    with st.sidebar:
        ...
        if st.button("Generate Plan 🚀", use_container_width=True):
            ...
            if is_valid:
                ...
                # --- ✳️ FIX: 'expand' changed to 'expanded' ---
                with st.status(f"Generating plan for {st.session_state.user_name}...", expanded=True) as status:
                    try:
                        ...
                        if not rag_results:
                            ...
                            if itinerary:
                                ...
                                st.error("The AI agent failed to generate a response. Please check API keys.")
...
def _set_session_state_on_generate(itinerary: dict, query: str, priorities: dict) -> None:
    """Writes to session state ONCE after generation."""
